#!/usr/bin/env python3
"""Verify the IRON VEIN ACT-III artifacts against the solution.
...
BUGGY = ROOT / "ACT-III.bin"
FIXED = ROOT / "ACT-III_fixed.bin"
BUGGY_SHA256 = (
    "a0f9206fdc4a33ecd24db15d179c31c40ec8abbc358489fe61a7d45dcf2d2b48"
)
FIXED_SHA256 = (
    "bac3e278ce9ba9d0c166065b0218e71f3e3eb2ff0f80fee5f439e93ab058fa0c"
)

CHANGES = (
    (0x74E9, 0xD1, 0xD0, "valve command authorization branch"),
    (0xA245, 0xB9, 0xB1, "covert beacon gate branch"),
    (0xA29F, 0xD1, 0xD0, "reserved-sector persistence gate branch"),
    (0xA30B, 0xB9, 0xB1, "logic bomb arming gate branch"),
)
...
def _check_sizes(buggy: bytes, fixed: bytes) -> list[bool]:
    ...
    """
    return [
        _report("ACT-III.bin exists", BUGGY.exists()),
        _report("ACT-III_fixed.bin exists", FIXED.exists()),
...
def _check_hashes(buggy: bytes, fixed: bytes) -> list[bool]:
    ...
    """
    return [
        _report("ACT-III.bin sha256", _sha256(buggy) == BUGGY_SHA256),
        _report("ACT-III_fixed.bin sha256", _sha256(fixed) == FIXED_SHA256),
    ]
...
def main() -> int:
    """
    Run every ACT-III artifact check.
